...
<script>
  ...
  const nameToSign = "Bartosz Bak";
  const letterAppearDelay = 80; // ms between letters appearing (faster)
  const animationDuration = 250; // ms for one letter to draw

  let displayedLetters = $state([]); // Array to hold letters as they are revealed
  ...
  onMount(() => {
    ...
    for (let i = 0; i < nameToSign.length; i++) {
      ...
      const letterKey = char === " " ? " " : key + caseSuffix; // e.g. "B_up", "a_lo", " "
      ...
      if (data) {
        setTimeout(() => {
          ...
          displayedLetters = [...displayedLetters, newLetter];
          ...
          setTimeout(() => {
            ...
            if (indexToAnimate !== -1) {
              ...
              updatedLetters[indexToAnimate] = {
                ...updatedLetters[indexToAnimate],
                animate: true,
              };
              displayedLetters = updatedLetters;
            }
          }, 10); // Minimal delay
        }, cumulativeDelay);

        cumulativeDelay += letterAppearDelay;
      } else {
        console.warn(`Letter data for "${char}" (${letterKey}) not found.`);
        cumulativeDelay += letterAppearDelay / 2; // Still add some delay for missing chars
      }
    }
  });
</script>
...
<style>
  ...
  .signature-main > div:not(.space) {
    ...
    /* Margins applied via inline style */
    position: relative; /* Needed for negative margins to overlap correctly */
    box-sizing: border-box;
  }

  /* Global needed because SVG path is inside dynamically rendered element */
  ...
  .space {
    ...
    height: 51px;
    vertical-align: bottom; /* Align with bottom of letters */
    box-sizing: border-box;
